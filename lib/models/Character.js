...
module.exports = class Character {
  ...
  static async addXp(character_name, xpToAdd) {
    console.log('adding', xpToAdd, 'to', character_name);
    const currentXp = this.getXp(character_name);
    const numberXp = Number(currentXp);
    console.log('bruh', rows[0].experience);
  ...
  static async getXp(character_name) {
    const { rows } = await pool.query(
      ...
      `
        SELECT *
        FROM Characters c
        WHERE c.character_name = 'Bonjo'`
    );
    const currentXp = Number(rows[0].experience);
    return currentXp;
  }
