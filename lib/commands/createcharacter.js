...
//when the user types /create character you will be met with multiple inputs and options for creating the character
module.exports = {
  data: new SlashCommandBuilder()
    .setName('createcharacter')
    .setDescription('Creates a character with the (pre-modified) stats and the given name.')
    //lines 10 through 18 is where the user will input their stats with a min value of 3 
    //and a max value of 18 and finally the name of their character
  ...
  async execute(interaction) {
    ...
    //things get weird if the user has more than one character so we limit it to only one at a time
    if (character_name !== null) {
      return interaction.reply('One character at a time, please.');
    }
    const class_row = new ActionRowBuilder()
      .addComponents(
        //this is a drop down menu when you are able to select your class with and brief description of each one.
    ...
    const race_row = new ActionRowBuilder()
      .addComponents(
        new StringSelectMenuBuilder()
          //this is the drop down menu for selecting your characters race 
    ...
    const background_row = new ActionRowBuilder()
      .addComponents(
        new StringSelectMenuBuilder()
          //below provides a drop down menu for the user to select their characters background
    ...
    await(interaction.reply('Select class, race, and background.'));
    ...
    //bot sends all the prompts at once as soon as /createcharacter s used
    const filter = i => {
      i.deferUpdate();
      return i.user.id === interaction.user.id;
      //bot waits to do anything until the filter is met
    };
      //three similar blocks: when the bot gets an input on the dropdown, it notes the value selected then deletes the dropdown to finalize the choice
    classMessage.awaitMessageComponent({ filter, componentType: 'SELECT_MENU', time: 60000 })
      .then(interaction => interaction.editReply('didn\'t receive class'))
    // eslint-disable-next-line no-unused-vars
      .catch(err => classMessage.delete());
    //err isn't used but err has to be somewhere in the catch
    raceMessage.awaitMessageComponent({ filter, componentType: 'SELECT_MENU', time: 60000 })
      .then(interaction => interaction.editReply('didn\'t receive race'))
      // eslint-disable-next-line no-unused-vars
      .catch(err => raceMessage.delete());
    backgroundMessage.awaitMessageComponent({ filter, componentType: 'SELECT_MENU', time: 60000 })
      .then(interaction => interaction.editReply('didn\'t receive background'))
      // eslint-disable-next-line no-unused-vars
